...
def compute_display_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if bb1_col and bb2_col and bb3_col and bb4_col:
        ...
        # keep df's index so the concatenation below stays aligned even for a filtered frame
        bb1, bb2, bb3, bb4 = (pd.Series(v, index=df.index) for v in (b1, b2, b3, b4))

    lib = df[lib_col].fillna("").astype(str).str.strip() if lib_col else pd.Series([""] * len(df), index=df.index)
    id_display = lib + "_" + bb1.astype(str) + "_" + bb2.astype(str) + "_" + bb3.astype(str) + "_" + bb4.astype(str)
    valid = ~lib.str.lower().isin(sorted(NA_LIB_VALUES))
    if id_col:
        known = known_libs_from(df[lib_col]) if lib_col else ()
        fallback = df[id_col].astype(str).map(lambda v: strip_lib_anywhere(v, known))
        id_display = id_display.where(valid, fallback)
    # LibID: NA for missing lib (was the string "nan" from astype(str))
    lib = lib.where(valid, "NA")
